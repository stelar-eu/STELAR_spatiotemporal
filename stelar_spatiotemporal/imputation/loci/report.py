...
from loci.overview import get_dataset_items
...
from loci.variables.render_timeseries import render_timeseries
from loci.variables.render_geospatial import render_geospatial
...
def get_report_structure(config: Settings, summary: dict, variables: Container = None) -> Root:
...
def get_render_map() -> Dict[str, Callable]:
    ...
    render_map = {
        ...
        "Geometry": render_geospatial
    }

    return render_map

def get_html_report(config: Settings, description: dict, variables: Container = None) -> str:
    report = get_report_structure(config, description, variables)
    with tqdm(
...
def to_json(config: Settings, description_set: dict) -> str:
...
def to_file(config: Settings, output: str, output_file: Union[str, Path], silent: bool = True) -> None:
